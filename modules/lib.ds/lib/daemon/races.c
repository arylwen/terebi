...
static void create() {
    ...
    if( !Races ){ 
	    Races = ([]);
	    //load some races
		AddRace("/secure/cfg/races/human", 1);
		write_file("log_gab", "races.c: added human\n");
		AddRace("/secure/cfg/races/dwarf", 1);
		AddRace("/secure/cfg/races/elf", 1);
	};
...
void AddRace(string file, int player) {
    ...
    foreach(string line in explode(read_file(file),"\n")){
        ...
        case "LIMB":
            ...
            limb[3] = map(explode(tmp_limb[3], ","), function(string str) {
                  //TODO terebi doesn't replace constans like A_SHIRT int x = to_int(str);
    ...
    varargs string array GetRaces(int player_only) {
        ...
        return filter(keys(Races), function(string race, int player_only) {
			  ...
			  write_file("log_gab", "races.c res "+res);
