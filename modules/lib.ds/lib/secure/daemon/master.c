...
private static void load_access(string cfg, mapping resource) {
    ...
    static void eventReset() {
        ...
        foreach(ob in obs) {
            ...
            f = bind( (: call_other, ob, "clean_up", ({ 0 }) :), ob );
            if( f ) catch(evaluate(f));
            if( !ob ) {
                y++;
                continue;
            }
			//to molify the terebi compiler that really expects a mixed array
			//as the thirs argument
			mixed *rn = ({ ResetNumber  });
            f = bind((: call_other, ob, "reset", rn :), ob);
