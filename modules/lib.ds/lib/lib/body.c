...
varargs int eventHealDamage(int x, int internal, mixed limbs){
	...
	//the terebi compiler incorrectly generates the class return rather than the interface
	//so this efun fails for subclasses
	//map(limbs, (: AddHealthPoints($(x), $1) :));
	
	foreach(string limb in limbs) AddHealthPoints(x, limb);
...
varargs int eventDie(mixed agent){
   ...
   */
    ...
    float AddStaminaPoints(mixed x){
        ...
        if( !intp(x) && !floatp(x) ){
            //TODO log rather than error out error("Bad argument 1 to AddStaminaPoints().\n "+x);
			write_file("log_gab","cannot add "+x+" as stamina points\n");
